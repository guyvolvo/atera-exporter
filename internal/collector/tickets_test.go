...
// fakeTickets models a mature tenant: tens of thousands of Resolved and Deleted
...
// The whole point of this collector. Resolved and Deleted can hold tens of
// thousands of tickets that will never change again; walking them runs to over a
// thousand requests and several minutes, which times out every cycle. They must
// be counted, never downloaded.
func TestClosedTicketsAreCountedNotWalked(t *testing.T) {
	fake := &fakeTickets{
		counts: map[string]int{
			"Open": 2, "Pending": 0, "Resolved": 12000, "Closed": 450, "Deleted": 9000,
		},
		live: map[string][]map[string]any{
			"Open": {
				ticket(1, "High", "Alice", time.Now().Add(-48*time.Hour)),
	...
	// Nowhere near the thousand-plus a naive walk would cost.
...
atera_tickets{status="Closed"} 450
atera_tickets{status="Deleted"} 9000
atera_tickets{status="Open"} 2
atera_tickets{status="Pending"} 0
atera_tickets{status="Resolved"} 12000
`
...
// express show up as a gap rather than vanishing.
...
func TestUnassignedTechnicianIsLabelled(t *testing.T) {
	fake := &fakeTickets{
		...
		live: map[string][]map[string]any{
			"Open": {
				ticket(1, "High", "Alice Cohen", time.Now()),
...
atera_open_tickets_by_technician{technician="Alice Cohen"} 1
...
func TestOldestOpenTicketWins(t *testing.T) {
	...
	fake := &fakeTickets{
		...
		live: map[string][]map[string]any{
			"Open": {
				ticket(1, "Low", "Alice", time.Now().Add(-time.Hour)),
				ticket(2, "High", "Alice", oldest),
				ticket(3, "Low", "Alice", time.Now().Add(-2*time.Hour)),
			},
		},
	}
	srv := httptest.NewServer(fake.handler())
