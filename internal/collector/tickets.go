...
//
// It never walks the ticket corpus. The estate holds ~59k tickets and the API
// caps itemsInPage at 50, so a full walk is 1,182 requests and takes minutes —
// it would time out on every cycle. Instead:
//
...
//
// A handful of tickets (4 of 59,077 as of 2026-07) match no ticketStatus value
// the filter accepts, so the per-status counts sum slightly below the unfiltered
// total. atera_tickets_total is exported precisely so that gap is visible rather
