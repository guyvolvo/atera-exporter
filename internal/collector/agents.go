...
//
// Aggregates are grouped by folder, not customer. This is a single-tenant estate
// where every agent carries the same CustomerName, so a per-customer breakdown
// would be one degenerate bucket; FolderName is the grouping that means anything.
type Agents struct {
	client   *atera.Client
