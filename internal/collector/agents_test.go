...
func TestRefreshWalksAllPages(t *testing.T) {
	fake := &fakeAtera{agents: []map[string]any{
		agent(1, "PC1", "Workstations", true),
		agent(2, "PC2", "Workstations", true),
		agent(3, "PC3", "Workstations", false),
		agent(4, "PC4", "Servers", true),
		agent(5, "PC5", "Servers", false),
	}}
...
atera_agents_total{folder="Workstations",online="false"} 0
atera_agents_total{folder="Workstations",online="true"} 2
`
...
func TestDuplicateMachineIDDoesNotBreakScrape(t *testing.T) {
	fake := &fakeAtera{agents: []map[string]any{
		agent(1, "PC1", "Workstations", true),
		agent(1, "PC1-dup", "Workstations", true),
	}}
	srv := httptest.NewServer(fake.handler())
	defer srv.Close()
...
atera_agent_info{agent_id="1",agent_version="",customer="Contoso",device_guid="guid-1",device_type="PC",folder="Workstations",hostname="PC1",ip="",model="",os="Microsoft Windows 11 Pro x64",os_version="",vendor=""} 1
`
